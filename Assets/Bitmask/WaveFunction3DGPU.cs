...
using UnityEngine;
using System;
using Tile3DStruct = WFCStructs.Tile3DStruct;
using Cell3DStruct = WFCStructs.Cell3DStruct;
...
public class WaveFunction3DGPU : MonoBehaviour
{
    ...
    [SerializeField] private Tile3D[] tileObjects;
    ...
    private void Start()
    {
        Initialize(new Vector3Int(dimensionsX, dimensionsY, dimensionsZ), cellSize, tileObjects);
    }
    ...
    public void Initialize(Vector3Int mapDimensions, float cellSize, Tile3D[] tiles)
    {
     ...
     void Generate()
    {
        ...
        //TestingCPU
        uint iterations = 0;
        while (iterations < dimensionsX * dimensionsY * dimensionsZ)
        {
    ...
    private void CheckNeighborsBitMask(uint x, uint y, uint z)
    {
        uint up, down, left, right, above, below;
        uint index = (uint)(x + (z * dimensionsX) + (y * dimensionsX * dimensionsZ));
        right = (uint)((x + 1) + (z * dimensionsX) + (y * dimensionsX * dimensionsZ));
        left = (uint)((x - 1) + (z * dimensionsX) + (y * dimensionsX * dimensionsZ));
        up = (uint)(x + ((z + 1) * dimensionsX) + (y * dimensionsX * dimensionsZ));
        down = (uint)(x + ((z - 1) * dimensionsX) + (y * dimensionsX * dimensionsZ));
        above = (uint)(x + (z * dimensionsX) + ((y + 1) * dimensionsX * dimensionsZ));
        below = (uint)(x + (z * dimensionsX) + ((y - 1) * dimensionsX * dimensionsZ));

        if(gridComponentsStructs[index].colapsed == 1) return;
        if(x < 0 || x >= dimensionsX || y < 0 || y >= dimensionsY || z < 0 || z >= dimensionsZ) return;
    ...
    Cell3DStruct[] CreateCell3DStructs()
    {
        ...
        for (int i = 0; i < gridComponents.Count; i++)
        {
            ...
            cellStruct.entropy = tileObjectIndexes.Length;

            ulong[][] propagationTable = new ulong[6][]; 
            int totalTiles = tileObjects.Length;

            // 0 Left, 1 Front, 2 Right, 3 Back, 4 Top, 5 Bottom
            for (int direction = 0; direction < 6; direction++) propagationTable[direction] = new ulong[totalTiles];
    ...
    void CreateSolidFloor(Cell3DStruct[] cell3DStructs)
    {
        int y = 0;
        for (int z = 0; z < dimensionsZ; z++)
        {
    ...
    /// </summary>
    /// <param name="cell3DStructs"></param>
    void CreateEmptyCeiling(Cell3DStruct[] cell3DStructs)
    {
        int y = dimensionsY - 1;
        for (int z = 0; z < dimensionsZ; z++)
        {

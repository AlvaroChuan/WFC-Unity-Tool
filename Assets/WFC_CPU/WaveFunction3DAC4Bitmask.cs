...
public class WaveFunction3DAC4Bitmask : MonoBehaviour
{
    ...
    private ulong[][] validNeighbors;                               
    private int cellSize;
    private int iterations = 0;
    private List<Cell3D> gridComponents;
    private Stopwatch stopwatch;
    
    // AC-4 Specific Structures
    ...
    private void GenerateMap()
    {
        ...
        stopwatch.Stop();
        print($"AC4 Bitmask Map generated completely in {stopwatch.ElapsedMilliseconds} ms ({stopwatch.ElapsedMilliseconds / 1000f} s)");
    }
    ...
    #region WFC Constraint Logic (AC-4 Bitmask)
    ...
    {
        if ((gridComponents[c].possibleTilesMask & (1UL << t)) != 0)
        {
            gridComponents[c].possibleTilesMask &= ~(1UL << t);
            gridComponents[c].entropy--;
            removals.Push((c, t));
        }
    }
    ...
    void Propagate()
    {
        while (removals.Count > 0)
        {
            ...
            // When tile tIndex is removed from cell c, it can no longer act as an enabler 
            ...
            for (int d = 0; d < 6; d++)
            {
                ...
                // The tiles in the neighbor (direction d) that are compatible with tIndex in cell c
                ulong compatibleInNeighbor = validNeighbors[d][tIndex];
                
                // Only consider tiles that are currently still possible in the neighbor
                ulong activeCompatibles = compatibleInNeighbor & gridComponents[neighbor].possibleTilesMask;
                
                // Iterate through all set bits
                for (int compIndex = 0; compIndex < tileObjects.Length; compIndex++)
                {
                    if ((activeCompatibles & (1UL << compIndex)) != 0)
                    {
    ...
    void CollapseTo(int c, Tile3D tile)
    {
        ...
        {
            if (t != targetIndex && (gridComponents[c].possibleTilesMask & (1UL << t)) != 0)
            {
                Ban(c, t);
            }
        }
    ...
    void InstantiateTile(int c, Tile3D tile)
    {
        ...
        cell.RecreateCell(cell.possibleTilesMask, cell.entropy);
    }
    ...
    bool CheckEntropy()
    {
        ...
        for (int c = 0; c < gridComponents.Count; c++)
        {
            ...
            {
                if (gridComponents[c].entropy < minOptions)
                {
                    minOptions = gridComponents[c].entropy;
                    bestCells.Clear();
                    bestCells.Add(c);
                }
                else if (gridComponents[c].entropy == minOptions)
                {
                    bestCells.Add(c);
                }
            }
        ...
        for (int t = 0; t < tileObjects.Length; t++)
        {
            if ((gridComponents[cellIndex].possibleTilesMask & (1UL << t)) != 0)
            {
    ...
    void InitializeGrid()
    {
        ...
        for (int c = 0; c < totalCells; c++)
        {
            for (int t = 0; t < tileObjects.Length; t++)
            {
                for (int d = 0; d < 6; d++)
                {
                    enablerCount[c, t, d] = CountBits(validNeighbors[d][t]);
                }
            }
        }
    }

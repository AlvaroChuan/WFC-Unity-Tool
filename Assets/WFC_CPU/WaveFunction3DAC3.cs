...
public class WaveFunction3DAC3 : MonoBehaviour
{
    ...
    private void GenerateMap()
    {
        ...
        stopwatch.Stop();
        print($"List-Based AC3 Map generated completely in {stopwatch.ElapsedMilliseconds} ms ({stopwatch.ElapsedMilliseconds / 1000f} s)");
    }
    ...
    #region WFC Constraint Logic (List-Based AC3)
    ...
    private bool PropagateConstraints()
    {
        ...
        while (propagationQueue.Count > 0)
        {
            ...
            bool changed = CheckNeighbours(x, y, z);
            
            if (gridComponents[index].tileOptions.Length == 0)
            {
    ...
    bool CheckNeighbours(int x, int y, int z)
    {
        int index = x + (z * dimensionsX) + (y * dimensionsX * dimensionsZ);
        
        if (gridComponents[index].collapsed) return false;

        int oldOptionCount = gridComponents[index].tileOptions.Length;
        // Start with the current valid options for this cell to optimize the checks
        ...
        if (options.Count != oldOptionCount)
        {
    ...
    bool CollapseCell(List<Cell3D> tempGrid)
    {
        ...
        // Enqueue neighbors to propagate the constraints of this new collapse

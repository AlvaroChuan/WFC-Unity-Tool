    ...
    private unsafe void InstantiateChunk()
    {
        for (int i = 0; i < gridComponentsStructs.Length; i++)
        {
            ...
            else
            {
                ...
                cell.RecreateCell(new Tile3D[] { tileObjects[gridComponentsStructs[i].tileOptions[0]] });
                ...
                Tile3D instantiatedTile = Instantiate(tileObjects[gridComponentsStructs[i].tileOptions[0]], cell.transform.position, Quaternion.identity, cell.transform);
                if (instantiatedTile.rotation != Vector3.zero)
                {
                    instantiatedTile.gameObject.transform.Rotate(tileObjects[gridComponentsStructs[i].tileOptions[0]].rotation, Space.Self);
                }
